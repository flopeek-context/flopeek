...
const { CANONICAL_REPOSITORY, EXPECTED_BASELINE, verifyImportSafety } = require("../../scripts/verify-import-safety");
...
test("current import provenance, canonical authority, and release blocks are consistent", () => {
  const result = verifyImportSafety(ROOT);
  assert.equal(result.status, "passed");
  assert.equal(result.sourceBaseline, EXPECTED_BASELINE);
  assert.equal(result.repositoryAuthority, "canonical");
  assert.equal(result.canonicalRepository, CANONICAL_REPOSITORY);
  assert.equal(result.productIdentity, "preserved");
  assert.equal(result.publicationAuthority, "pending");
  assert.equal(result.npmPublication, "blocked");
  assert.equal(result.githubRelease, "blocked");
...
test("import safety rejects publishable package metadata and release mutations", (context) => {
  ...
  assert.throws(() => verifyImportSafety(root), /canonical Flopeek package must remain private/);
  ...
  assert.throws(() => verifyImportSafety(root), /disabled promotion workflow still contains mutating command: npm publish/);
});
