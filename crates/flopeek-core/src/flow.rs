...
mod tests {
    ...
    fn flow_caps_and_cycles_are_deterministic_with_categorical_omissions() {
        ...
        let edge =
            derive(&root, "project_test", &edge_files, &edge_nodes, &edge_edges).expect("edge");
    ...
    fn related_tests_keep_only_proven_relations_and_strengths() {
        ...
        assert!(related.records.iter().any(|record| {
            record.relation == "direct-construct" && record.strength == "strong"
        }));
        assert!(
            related
                .records
